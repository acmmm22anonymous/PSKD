...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default="./output_firststage", type=str, help='Path to save logs and checkpoints.')
...
def prepare_dataset(args):
    ...
    dataset_train_skt = MyImageFolder(os.path.join(args.data_path,  'sketch'), train_classes,
                                      outlier=outliers,
                                      transform=transform)
    dataset_train_img = MyImageFolder(os.path.join(args.data_path,  'photo'), train_classes,
                                      outlier=None,
                                      transform=transform)
...
def train_dino(args):
    ...
    fp16_scaler = None
    if args.use_fp16:
        fp16_scaler = torch.cuda.amp.GradScaler()  # 动态损失放大
    ...
    if not args.resume_pretrain:
        ...
        utils.restart_from_checkpoint_fdwang(
        ckp_stu_name,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, lsce_loss, 
        ...
        if utils.is_main_process():  # 记录每一个epoch更新的损失值，并且参数名字前加上train_前缀
...
def train_one_epoch(student,
                    lsce_loss, data_loader_img, data_loader_skt,
        ...
        for i in range(grad_acc_steps):
            ...
            with my_context():
                ...
                with torch.cuda.amp.autocast(fp16_scaler is not None):
                    student_output_img_f, student_output_img_f_fea = student(sub_images, 'img')
                    ...
                    if (i+1) % args.skt_factor == 0:
                        ...
                        student_output_skt_f, student_output_skt_f_fea = student(sub_sketches, 'skt')
                ...
                if not math.isfinite(loss.item()):  # nan或无穷大
