...
def restart_from_checkpoint_fd2(ckp_tea_path,  ckp_stu_path, teacher, student, run_variables=None):
    """
    Re-start from checkpoint
    """
    if not os.path.isfile(ckp_tea_path) or not os.path.isfile(ckp_stu_path):
        print('=> there is no checkpoint for the specified file')
        return
    print("Found checkpoint at {}, {}".format(ckp_tea_path,ckp_stu_path))

    # open checkpoint file
    checkpoint_stu = torch.load(ckp_stu_path, map_location="cpu")
    checkpoint_tea = torch.load(ckp_tea_path, map_location='cpu')
    ...
    def load_state_dict(checkpoint, model, ckp_path, ckp_name):
        try:
            ...
            try:
                ...
                print("=> failed to load '{}' from checkpoint: '{}'".format(ckp_name, ckp_path))
    load_state_dict(checkpoint_tea, teacher, ckp_tea_path, 'teacher')
    load_state_dict(checkpoint_stu, student, ckp_stu_path, 'student')
def restart_from_checkpoint_fd3(ckp_stu_path, student, run_variables=None):
    """
    Re-start from checkpoint
    """
    if not os.path.isfile(ckp_stu_path):
        print('=> there is no checkpoint for the specified file')
        return
    print("Found checkpoint at {}, ".format(ckp_stu_path))
...
def cosine_scheduler_loss(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0, early_schedule_epochs=0):
    ...
    if early_schedule_epochs == 0:
        ...
        iters = np.arange(early_schedule_epochs * niter_per_ep - warmup_iters)
        # note: for schedule, the second line is the same with other schedule,
        # but the first line is actually used in experiment and perform well;
        # for the first line, the result will be the almost constant: 1/2 * (final_value+base_value)
        # schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(np.pi * iters) / len(iters))
        schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(np.pi * iters / len(iters)))
        remainder = np.array([final_value]*((epochs - early_schedule_epochs) * niter_per_ep))
        schedule = np.concatenate((warmup_schedule, schedule, remainder))
...
class FDMultiCropWrapper_3(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, head, head_skt, head_fusion):
        super(FDMultiCropWrappe_3, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        self.token_num = backbone.token_num
    ...
    def forward(self, x, modality='img'):
        ...
        for end_idx in idx_crops:
            ...
            start_idx = end_idx
        # Run the head forward on the concatenated features.
        f_cls, f_fea,prototypes = self.head_fusion(output_fusion, return_both=True)
        if modality == 'img':
            return f_cls, prototypes
        elif modality == 'skt':
            return f_cls, prototypes
        else:
            print('You must specifiy the modality!')
...
def save_qualitative_results(root, sketch_dir, photo_dir, fls_sk, fls_im, dir_op, aps, sim,
                             str_sim, time_suffix, nq=50, nim=20, im_sz=(256, 256), best=False, save_image=False):

    # Set directories according to dataset
    # dir_sk = os.path.join(root, sketch_dir)
    # dir_im = os.path.join(root, photo_dir)
    ...
    if not os.path.isdir(dir_op):
        ...
        pass
        clean_folder(dir_op)  # 如果存在，则清空该文件夹

    if best:
        ind_sk = np.argsort(-np.array(aps))[:nq]
    else:
        np.random.seed(0)
        ind_sk = np.random.choice(len(aps), nq, replace=False)

    # create a text file for results
    fp = open(os.path.join(dir_op, "Results-"+time_suffix+".txt"), "w")
    ...
    for i, isk in enumerate(ind_sk):
        ...
        if save_image:
            ...
            if not os.path.isdir(sdir_op):
                os.makedirs(sdir_op)
            sk = Image.open(os.path.join('', fls_sk[isk])).convert(mode='RGB').resize(im_sz)
            sk.save(os.path.join(sdir_op, fls_sk[isk].split('/')[-1]))
        ...
        for j, iim in enumerate(ind_im):
            if j < len(ind_im)-1:
                ...
                fp.write("{0} {1}".format(fls_im[iim], str_sim[isk][iim]))
            if save_image:
                im = Image.open(os.path.join('', fls_im[iim])).convert(mode='RGB').resize(im_sz)
                im.save(os.path.join(sdir_op, str(j + 1) + '_' + str(str_sim[isk][iim]) + '.jpg'))
        fp.write("\n")
    fp.close()
...
def compressITQ(Xtrain, Xtest, n_iter=50):
    ...
    Ctrain = Ctrain > 0
    Ctest = Ctest > 0
    # 根据最后算得的元素值是否大于零来获取binary embedding,如果元素大于零，则取1；小于零，则取0。
