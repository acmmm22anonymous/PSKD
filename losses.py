...
def KOLEO_loss(x, pdist=nn.PairwiseDistance(2)):
    '''
    https://github.com/facebookresearch/spreadingvectors/blob/7aae7398e9683fc26d7fb2bbeb15eb44bfee8021/train.py#L25
    :param x:
    :param pdist:
    :return:
    '''
...
def AdaptationLoss(prototypes,input, target, smoothing=0.1):
    '''
    :param input: logits
    :param target: shape like a list
    :param smoothing:
    :return:
    '''
    log_prob =F.log_softmax(torch.mm(prototypes,input.T),dim=1)
    weight = log_prob.new_ones(log_prob.size()) * smoothing / (log_prob.size(-1) - 1.)
    weight.scatter_(-1, target.unsqueeze(-1), (1. - smoothing))
    loss = (-weight * log_prob).sum(dim=-1).mean()
    return loss


class AdaptationLossDINO(nn.Module):
    def __init__(self, num_crops):
        super(AdaptationLossDINO, self).__init__()
        self.num_crops = num_crops

    def forward(self, prototypes,input, target):
        input = input.chunk(self.num_crops)
        total_loss = 0
        n_loss_terms = 0
        for i in range(len(input)):
            loss = AdaptationLoss(prototypes,input[i], target)
            total_loss += loss.mean()
            n_loss_terms += 1
        total_loss /= n_loss_terms
        return total_loss