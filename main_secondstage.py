...
def train_dino(args):
    ...
    args.arch = args.arch.replace("deit", "vit")  # 将基础架构中deit名字换成vit
        ...
        for i in range(grad_acc_steps):
            ...
            with my_context():
                ...
                with torch.cuda.amp.autocast(fp16_scaler is not None):
                    ...
                    student_output_img_f, prototypes_img = student(sub_images, 'img')
                    ...
                    loss = tra_kd_loss(student_output_img_f,teacher_output_img_f,class_similarity )+lsce_loss(student_output_img_f, sub_lab_img)+ce_loss(moveset_img,target_img)
                    ...
                    if (i+1) % args.skt_factor == 0:
                        ...
                        student_output_skt_f,prototypes_skt = student(sub_sketches, 'skt')
                        ...
                        loss = tra_kd_loss(student_output_skt_f,teacher_output_skt_f,class_similarity )+lsce_loss(student_output_skt_f, sub_lab_skt)+ce_loss(moveset_skt,target_skt)
                        loss /= (grad_acc_steps + grad_acc_steps / args.skt_factor)
                        loss_tmp += loss.item()
                        if fp16_scaler is None:
                            loss.backward()
